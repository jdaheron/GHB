...
/* Includes ------------------------------------------------------------------*/
#include <ETH/lwip-1.4.1/port/STM32F2x7/Standalone/ethernetif.h>
...
* @retval None
*/
void LwIP_Init(uint8_t* Address, uint8_t* Mask, uint8_t* Gateway, Bool_e DHCP_Active, uint8_t* MAC)
{
  ...
  netif_add(&gnetif, &ipaddr, &netmask, &gw, NULL, &ethernetif_init, &ethernet_input, MAC);
