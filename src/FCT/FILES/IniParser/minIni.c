...
#include <FILES/IniParser/minIni.h>
#if defined NDEBUG
  #define assert(e)
#else
  #include <assert.h>
...
int  ini_getsection(int idx, TCHAR *Buffer, int BufferSize, const TCHAR *Filename)
{
	...
	if (ini_openread(Filename, &fp))
	{
		nb = ini_getsection_OpenedFile(idx, Buffer, BufferSize, &fp);
		(void) ini_close(&fp);
	}
...
)
{
	...
	ok = getkeystring(pFile, NULL, NULL, idx, -1, Buffer, BufferSize);
...
)
{
	...
	(void) ini_seek(pFileRead, &mark);
	(void) ini_seek(pFileWrite, &mark);

	//	assert(Filename != NULL);
	//	if (!ini_openread(Filename, pFile))
	//	{
	//		/* If the .ini file doesn't exist, make a new file */
	//		if (Key != NULL && Value != NULL)
	//		{
	...
	if (Key != NULL && Value != NULL)
	{
		(void) ini_tell(pFileRead, &mark);
		match = getkeystring(pFileRead, Section, Key, -1, -1, LocalBuffer, sizearray(LocalBuffer));
		if (match && _tcscmp(LocalBuffer, Value) == 0)
		{
			//TODO f_sync(pFileWrite);
			return 0;
		}
	...
	if (len > 0)
	{
		do
		{
			if (!ini_read(LocalBuffer, INI_BUFFERSIZE, pFileRead))
			{
				/* Failed to find section, so add one to the end */
				flag = cache_flush(LocalBuffer, &cachelen, pFileRead, pFileWrite, &mark);
				if (Key != NULL && Value != NULL)
				{
					if (!flag)
					{
						//(void) ini_write(INI_LINETERM, pFileWrite); /* force a new line behind the last line of the INI file */

					}
					//_printf("nl0\n");
					(void) ini_write(INI_LINETERM, pFileWrite); /* force a new line behind the last line of the INI file */
					writesection(LocalBuffer, Section, pFileWrite);
					writekey(LocalBuffer, Key, Value, pFileWrite);
				} /* if */

				f_sync(pFileWrite);
				return 1;//TODO close_rename(pFileRead, pFileWrite, Filename, LocalBuffer); /* clean up and rename */
			} /* if */
			/* Copy the line from source to dest, but not if this is the section that
			 * we are looking for and this section must be removed
			 */
			...
			if (!match || Key != NULL)
			{
				if (!cache_accum(LocalBuffer, &cachelen, INI_BUFFERSIZE))
				{
					cache_flush(LocalBuffer, &cachelen, pFileRead, pFileWrite, &mark);
					(void) ini_read(LocalBuffer, INI_BUFFERSIZE, pFileRead);
					cache_accum(LocalBuffer, &cachelen, INI_BUFFERSIZE);
				} /* if */
			} /* if */
		} while (!match);
	...
	for (;;)
	{
		...
		if (Key == NULL)
		{
			(void) ini_tell(pFileRead, &mark); /* we are deleting the entire section, so update the read position */
		}
		else
		{
			if (!cache_accum(LocalBuffer, &cachelen, INI_BUFFERSIZE))
	...
	/* Copy the rest of the INI file */
	while (ini_read(LocalBuffer, INI_BUFFERSIZE, pFileRead))
	{
		if (!cache_accum(LocalBuffer, &cachelen, INI_BUFFERSIZE))
		{
			cache_flush(LocalBuffer, &cachelen, pFileRead, pFileWrite, &mark);
	...
	f_sync(pFileWrite);//TODO voir si necessaire
	return 1;
}
