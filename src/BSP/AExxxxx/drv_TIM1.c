...
void TIM1_Init(uint32_t Periode_us) {
	...
	// Activation Timer
	TIM_Cmd(TIM1, ENABLE);
	TIM_CtrlPWMOutputs(TIM1, ENABLE); // TIM1 et TIM8 seulement
...
void TIM1_DeInit() {
	...
	TIM_Cmd(TIM1, DISABLE);								// Desactivation peripherique
	RCC_APB2PeriphResetCmd(RCC_APB2Periph_TIM1, ENABLE);	// Desactivation horloge
...
{
	if (MappingGpio[PinId].EtatInit == TIM_CCx_Enable)
		TIM_CCxCmd(TIM1, MappingGpio[PinId].PARAM, MappingGpio[PinId].EtatInit);
	else
		TIM_CCxNCmd(TIM1, MappingGpio[PinId].PARAM, MappingGpio[PinId].EtatInit);
}
...
{
	if (MappingGpio[PinId].EtatInit == TIM_CCx_Enable)
		TIM_CCxCmd(TIM1, MappingGpio[PinId].PARAM, TIM_CCx_Disable);
	else
		TIM_CCxNCmd(TIM1, MappingGpio[PinId].PARAM, TIM_CCxN_Disable);
}
