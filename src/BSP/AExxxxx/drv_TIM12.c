...
void TIM12_DeInit() {
	...
	TIM_Cmd(TIM12, DISABLE);								// Desactivation peripherique
	RCC_APB1PeriphResetCmd(RCC_APB1Periph_TIM12, ENABLE);	// Desactivation horloge
...
void TIM12_PWM_Configurer(MappingGpio_e PinId, Etat_e Etat, uint8_t Ratio_pr100)
{
	...
	switch (MappingGpio[PinId].PARAM)
	{
		case TIM_Channel_1:
			...
			TIM_OC1Init(TIM12, &TIM_OCInit);
			break;

		case TIM_Channel_2:
			SaveRatio_pr100[1] = Ratio_pr100;
			TIM_OC2Init(TIM12, &TIM_OCInit);
			break;

		case TIM_Channel_3:
			SaveRatio_pr100[2] = Ratio_pr100;
			TIM_OC3Init(TIM12, &TIM_OCInit);
			break;

		case TIM_Channel_4:
			SaveRatio_pr100[3] = Ratio_pr100;
			TIM_OC4Init(TIM12, &TIM_OCInit);
			break;
...
{
	TIM_CCxCmd(TIM12, MappingGpio[PinId].PARAM, TIM_CCx_Enable);
	//TIM_CtrlPWMOutputs(TIM12, ENABLE); // TIM1 et TIM8 seulement
}
...
{
	TIM_CCxCmd(TIM12, MappingGpio[PinId].PARAM, TIM_CCx_Disable);
	//TIM_CtrlPWMOutputs(TIM12, DISABLE); // TIM1 et TIM8 seulement
}
