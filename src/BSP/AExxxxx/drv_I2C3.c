...
void I2C3_DeInit()
{
	I2C_Cmd(I2C3, DISABLE);									// Desactivation peripherique
	RCC_APB1PeriphResetCmd(RCC_APB1Periph_I2C3, ENABLE);	// Desactivation horloge
}
...
uint8_t I2C3_ReadByte(uint8_t SlaveAddress, uint8_t DataAddress)
{
	...
	I2C3_ReadBuffer(SlaveAddress, DataAddress, &Data, 1);
...
uint16_t I2C3_ReadWord(uint8_t SlaveAddress, uint8_t DataAddress)
{
	...
	I2C3_ReadBuffer(SlaveAddress, DataAddress, Data, 2);
	...
	// Wait EV5
	TO = TO_Value;
	while ((!I2C_CheckEvent(I2C3, I2C_EVENT_MASTER_MODE_SELECT)) && (TO--));
	...
	// Wait EV8
	TO = TO_Value;
	while ((!I2C_CheckEvent(I2C3, I2C_EVENT_MASTER_BYTE_TRANSMITTING)) && (TO--));

	// COMMAND CODE
	I2C_SendData(I2C3, DataAddress);

	// Wait EV8_2
	TO = TO_Value;
	while ((!I2C_CheckEvent(I2C3, I2C_EVENT_MASTER_BYTE_TRANSMITTED)) && (TO--));
	...
	I2C_NACKPositionConfig(I2C3, I2C_NACKPosition_Next);
	Data  = I2C_ReceiveData(I2C3);

	// Wait EV7
	TO = TO_Value;
	while ((!I2C_CheckEvent(I2C3, I2C_EVENT_MASTER_BYTE_RECEIVED)) && (TO--));

	// READ WORD (MSB)
	I2C_AcknowledgeConfig(I2C3, ENABLE);
	I2C_NACKPositionConfig(I2C3, I2C_NACKPosition_Current);
	Data += (I2C_ReceiveData(I2C3) << 8) & 0xFF00;
	...
	TO = TO_Value;
	while ((!I2C_GetFlagStatus(I2C3, I2C_FLAG_RXNE)) && (TO--));
...
void I2C3_ReadBuffer(uint8_t SlaveAddress, uint8_t DataAddress, uint8_t* pData, uint16_t Taille)
{
	...
	// Wait EV6
	TO = TO_Value;
	while ((!I2C_CheckEvent(I2C3, I2C_EVENT_MASTER_RECEIVER_MODE_SELECTED)) && (TO--));
	...
	while (Taille > 0)
	{
		...
		{
			I2C_AcknowledgeConfig(I2C3, DISABLE);	// Pas d'envoi ACK
		...
		pData[iBuffer] = I2C_ReceiveData(I2C3);
 ...
 */
void I2C3_WriteByte(uint8_t SlaveAddress, uint8_t DataAddress, uint8_t Data)
{
	I2C3_WriteBuffer(SlaveAddress, DataAddress, &Data, 1);
}
 ...
 * @param[in]	Data			Donnee a ecrire.
 */
void I2C3_WriteWord(uint8_t SlaveAddress, uint8_t DataAddress, uint16_t Data)
{
#if 1
	...
	I2C3_WriteBuffer(SlaveAddress, DataAddress, buffer, 2);
	...
	// Wait EV6
	TO = TO_Value;
	while ((!I2C_CheckEvent(I2C3, I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED)) && (TO--));

	// COMMAND CODE
	I2C_SendData(I2C3, SlaveAddress);
	...
	// WRITE WORD (LSB)
	I2C_SendData(I2C3, (uint8_t) (Data & 0x00FF));
	...
	// WRITE WORD (MSB)
	I2C_SendData(I2C3, (uint8_t) ((Data >> 8) & 0x00FF));
	...
	I2C_GenerateSTOP(I2C3, ENABLE);
	TO = TO_Value;
	while ((!I2C_GetFlagStatus(I2C3, I2C_FLAG_TXE)) && (TO--));
...
void I2C3_WriteBuffer(uint8_t SlaveAddress, uint8_t DataAddress, uint8_t* pData, uint16_t Taille)
{
	...
	while (Taille > 0)
	{
		I2C_SendData(I2C3, pData[iBuffer]);
